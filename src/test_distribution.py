...
def test_multiplicative_normal():
    ...
    c = positive_definite_matrix([time, dim, dim])
    m = np.random.normal(0, 1, [time, dim])
    # Compute intermediate results.
    diag = np.array([np.eye(dim) for i in range(time)])
    offdiag = np.array([-a for i in range(time - 1)])
    dense_eye_minus_a_inv = np.linalg.inv(dense_matrix(diag, offdiag))
    dense_q = dense_matrix(np.array([q1] + [q for i in range(time - 1)]))
    dense_c = dense_matrix(c)
    cov_inv = np.linalg.inv(np.matmul(
            np.matmul(dense_eye_minus_a_inv.T, dense_q),
            dense_eye_minus_a_inv)) + dense_c

    cov = np.linalg.inv(cov_inv)
    mu = np.matmul(np.matmul(cov, dense_c), m.reshape([dim * time, 1]))
    ...
    c = positive_definite_matrix([n_dist, time, dim, dim])
